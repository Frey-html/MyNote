...
void GenTableList()
{
	tableList[0] = glGenLists(2);
	tableList[1] = tableList[0] + 1;

	glNewList(tableList[0], GL_COMPILE);
	DrawTable();
	glEndList();

	glNewList(tableList[1], GL_COMPILE);
	glPushMatrix();
	glTranslatef(0, 0, 5);
	DrawTable();
	glPopMatrix();
...
{
	if (!bDrawAnotherTable) glCallList(tableList[0]);
	else
	{
		glCallList(tableList[0]);
...
void key(unsigned char k, int x, int y)
{
	...
	case '+':
	{
		bDrawAnotherTable = !bDrawAnotherTable;
	}
	default: break;
	}
}
...
int main (int argc,  char *argv[])
{
	...
	GenTableList();
	std::cout << tableList[0] << " " << tableList[1] << std::endl;
	//GenLightList();
